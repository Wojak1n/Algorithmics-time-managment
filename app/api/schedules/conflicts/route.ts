  ...
  try {
    ...
    for (const slot of timeSlots) {
      ...
      for (const conflict of teacherConflicts) {
        conflicts.push({
          type: 'teacher',
          message: `Teacher ${course.teacher.user.name} is already scheduled for ${conflict.course.subject.code} on ${slot.day} at ${slot.time}`,
      ...
      if (course.roomId) {
        ...
        for (const conflict of roomConflicts) {
          conflicts.push({
            type: 'room',
            message: `Room ${course.room?.name} is already booked for ${conflict.course.subject.code} on ${slot.day} at ${slot.time}`,
            conflictingCourse: conflict.course.name
          });
        }
      }
      ...
      for (const conflict of groupConflicts) {
        conflicts.push({
          type: 'group',
          message: `Group ${course.group.name} already has ${conflict.course.subject.code} scheduled on ${slot.day} at ${slot.time}`,
          conflictingCourse: conflict.course.name
        });
      }
    }
